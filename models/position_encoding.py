...
class PositionEmbeddingSine(nn.Module):
    ...
    """
    def __init__(self, num_pos_feats=64, temperature=20, normalize=False, scale=None):
        super().__init__()
...
def gen_sineembed_for_position(pos_tensor, temperature=20):
    ...
    dim_t = temperature ** (2 * (dim_t // 2) / 128) # torch.Size([128])

    x_embed = pos_tensor[:, :, 0] * scale #[num_q, bs]
    y_embed = pos_tensor[:, :, 1] * scale
    pos_x = x_embed[:, :, None] / dim_t #[num_q, bs, 128]
    pos_y = y_embed[:, :, None] / dim_t
...
@torch.no_grad()
def gen_sineembed_for_single_position(pos_tensor, temperature=20):
    scale = 2 * math.pi
    dim_t = torch.arange(128, dtype=torch.float32, device=pos_tensor.device)
    dim_t = temperature ** (2 * (dim_t // 2) / 128)
    tensor_embed = pos_tensor * scale
    pos = tensor_embed[:, :, None] / dim_t
