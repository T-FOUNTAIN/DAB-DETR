...
def get_window_obj(info, windows, iof_thr):
    ...
    # lock.acquire()
    # obj.append(bboxes.shape[0])
    # if bboxes.shape[0] > 0:
    #     box.append(sqrt(bt.bbox_areas(bboxes).mean()))
    # lock.release()
...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
    ...
    for i in range(windows.shape[0]):
        ...
        # bboxes = ann['bboxes']
        # lock.acquire()
        # pobj.append(bboxes.shape[0])
        # if bboxes.shape[0] > 0:
        #     pbox.append(sqrt(bt.bbox_areas(bboxes).mean()))
        # lock.release()
        ...
        patch_info['padding_mask'] = padding_mask = [-1, -1] # 表示实际部分的右下坐标,(-1,-1)表示未被填充
        ...
        if not no_padding:
            ...
            if height > patch.shape[0] or width > patch.shape[1]:
                ...
                padding_mask = [patch.shape[0], patch.shape[1]]
                patch = padding_patch
        patch_info['height'] = patch.shape[0]
        patch_info['width'] = patch.shape[1]
        patch_info['padding_mask'] = padding_mask
...
def main():
    ...
    # object_distribution = manager.list([])
    # box_average = manager.list([])
    # patch_object_distribution = manager.list([])
    # patch_box_average = manager.list([])
    ...
    worker = partial(single_split,
                     ...
                     total=len(infos),
                     logger=logger)
                     # objdis = object_distribution,
                     # boxav = box_average,
                     # pobjdis = patch_object_distribution,
                     # pboxav = patch_box_average)
    ...
    # print('config:', args.iof_thr, sizes)
    # object_distribution = np.array(object_distribution)
    # object_distribution_0 = len(np.where(object_distribution==0)[0])
    # object_distribution_1_100 = len(np.where((object_distribution>0)&(object_distribution<=100))[0])
    # object_distribution_101_200 = len(np.where((object_distribution>100)&(object_distribution<=200))[0])
    # object_distribution_201_300 = len(np.where((object_distribution>200)&(object_distribution<=300))[0])
    # object_distribution_301_400 = len(np.where((object_distribution>300)&(object_distribution<=400))[0])
    # object_distribution_401_500 = len(np.where((object_distribution>400)&(object_distribution<=500))[0])
    # object_distribution_500 = len(np.where(object_distribution > 500)[0])
    # print('global_object_distribution:')
    # print('0 object:',  object_distribution_0, '1-100 object:', object_distribution_1_100, '100-200 object:', object_distribution_101_200)
    # print('200-300 object:',  object_distribution_201_300, '300-400 object:', object_distribution_301_400, '400-500 object:', object_distribution_401_500)
    # print('500+ objects', object_distribution_500)


    # patch_object_distribution = np.array(patch_object_distribution)
    # patch_object_distribution_0 = len(np.where(patch_object_distribution==0)[0])
    # patch_object_distribution_1_100 = len(np.where((patch_object_distribution>0)&(patch_object_distribution<=100))[0])
    # patch_object_distribution_101_200 = len(np.where((patch_object_distribution>100)&(patch_object_distribution<=200))[0])
    # patch_object_distribution_201_300 = len(np.where((patch_object_distribution>200)&(patch_object_distribution<=300))[0])
    # patch_object_distribution_301_400 = len(np.where((patch_object_distribution>300)&(patch_object_distribution<=400))[0])
    # patch_object_distribution_401_500 = len(np.where((patch_object_distribution>400)&(patch_object_distribution<=500))[0])
    # patch_object_distribution_500 = len(np.where(patch_object_distribution>500)[0])
    # print('patch_object_distribution:')
    # print('0 object:',  patch_object_distribution_0, '1-100 object:', patch_object_distribution_1_100, '100-200 object:', patch_object_distribution_101_200)
    # print('200-300 object:',  patch_object_distribution_201_300, '300-400 object:', patch_object_distribution_301_400, '400-500 object:', patch_object_distribution_401_500)
    # print('500+ objects', patch_object_distribution_500)


    # box_average = np.array(box_average)
    # box_average_32 = len(np.where(box_average<=32)[0])
    # box_average_64 = len(np.where((box_average>32)&(box_average<=64))[0])
    # box_average_128 = len(np.where((box_average>64)&(box_average<=128))[0])
    # box_average_256 = len(np.where((box_average>128)&(box_average<=256))[0])
    # box_average_512 = len(np.where((box_average>256)&(box_average<=512))[0])
    # box_average_512_plus = len(np.where(box_average>512)[0])
    # print('box mean size per image:')
    # print('size 32:',  box_average_32, 'size 64:',  box_average_64, 'size 128:',  box_average_128, 'size 256:',  box_average_256)
    # print('size 512:',  box_average_512, 'size 512+:',  box_average_512_plus)

    # patch_box_average = np.array(patch_box_average)
    # patch_box_average_32 = len(np.where(patch_box_average<=32)[0])
    # patch_box_average_64 = len(np.where((patch_box_average>32)&(patch_box_average<=64))[0])
    # patch_box_average_128 = len(np.where((patch_box_average>64)&(patch_box_average<=128))[0])
    # patch_box_average_256 = len(np.where((patch_box_average>128)&(patch_box_average<=256))[0])
    # patch_box_average_512 = len(np.where((patch_box_average>256)&(patch_box_average<=512))[0])
    # patch_box_average_512_plus = len(np.where(patch_box_average>512)[0])
    # print('box mean size per patch:')
    # print('size 32:',  patch_box_average_32, 'size 64:',  patch_box_average_64, 'size 128:',  patch_box_average_128, 'size 256:',  patch_box_average_256)
    # print('size 512:',  patch_box_average_512, 'size 512+:',  patch_box_average_512_plus)
